...
CapacitiveSensor sensor3 = CapacitiveSensor(8, 7);//sensor3
CapacitiveSensor sensor4 = CapacitiveSensor(13, 12);////sensor4
...
void loop() {
  ...
  long measurement3 =  sensor3.capacitiveSensor(10);//讀取SENSOR3的數值
  long measurement4 =  sensor4.capacitiveSensor(10);//讀取SENSOR4的數值

  Serial.print(measurement);//SERIAL PRINT SENSOR1的數值以方便MAPPING
  Serial.print("\t");
  Serial.print(measurement2);//SERIAL PRINT SENSOR2的數值以方便MAPPING
  Serial.print("\t");
  Serial.print(measurement3);//SERIAL PRINT SENSOR3的數值以方便MAPPING
  Serial.print("\t");
  Serial.print(measurement4);//SERIAL PRINT SENSOR4的數值以方便MAPPING
  Serial.println("\t");
...
if (measurement >= 400){//決定觸發起動歌曲的條件(值)
  ...
  if (measurement4 >= 400){//決定觸發起動歌曲的條件(值)
